 ...
 *
 * V2-19 只改质感，不改版式：KPI 四数横排、62/38 分栏、漏斗与资源各 5 格并排
 * 全部保持原样，每块外面包一层 .board-card（矩形框 + 阴影）。
 * 节标题留在卡外，原来的阅读节奏不变。
 */
            ...
            {d.tasks.length === 0 ? (
              ...
              d.tasks.map(tk => (
                <Link key={tk.opp_id} href={`/opportunity/${tk.opp_id}`}
                      className="row flex items-center gap-4 h-11 px-4 hairline-b last:border-b-0">
                  <span className={`num text-micro w-[52px] shrink-0
            ...
            {d.reports.length === 0 ? (
              ...
              d.reports.map(r => (
                <Link key={r.id} href={`/research/${r.id}`}
                      className="row block py-3 px-4 hairline-b last:border-b-0">
        ...
        <div className="board-card grid-pair overflow-hidden grid grid-cols-2 md:grid-cols-5 gap-px bg-hairline">
          {d.funnel.map(f => (
            <div key={f.stage} className="bg-surface p-4">
              ...
              <div className="text-micro text-fg-subtle">{stageLabel(t, f.stage)}</div>
            </div>
          ))}
        </div>
        {/* IA §4.1 标注过：驻留天数与转化率需 stage_change 事件积累存量才能算。
            机会台的行内改阶段已在写这类事件，跑一段时间后在这里补上。 */}
        <p className="text-micro text-fg-faint mt-2.5">{t.overview.funnelEmpty}</p>
      </section>
        ...
        <div className="board-card grid-pair overflow-hidden grid grid-cols-2 md:grid-cols-5 gap-px bg-hairline">
          {d.resources.by_kind.map(k => (
            <div key={k.kind} className="bg-surface p-4">
              ...
              <div className="num text-micro text-fg-faint mt-0.5">{mb(k.bytes)} MB</div>
            </div>
          ))}
        </div>
      </section>
...
function Blank({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="flex flex-col items-center justify-center gap-2 py-10 text-fg-faint">
