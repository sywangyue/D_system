...
## P1-5 first_year：未执行，需你重新裁决

你的决定是「历史遗留，后续不开发，直接删掉」。但迁移 `010_first_year.sql` 的注释写明：
...
删列 = 把刚修好的打标工具链再打断一次。三个选项：

- **A 保留列，改文档**（推荐）—— AGENTS.md 把 `first_year` 从「自动填充（爬虫）」移到「待填」
- **B 删列 + 同步删掉 4 个文件里的引用** —— 打标 Excel 少一列，dedup 少一个合并信号
- **C 保留列并回填** —— 从 `raw_jufair` 的首届年份推，需要另立一批
