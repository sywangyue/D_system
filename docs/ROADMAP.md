...
| 编码 | 内容 | 说明 |
|---|---|---|
| V2-18 | **落地页设计 + 数据模型盘点**（阶段 A） | Stitch → Claude Design → DeepSeek 落地。落地页不依赖业务数据，可立即开工；并行盘点归档所需的字段缺口 |
| V2-19 | **录入通道 + 项目归档**（阶段 B/C） | 先收敛重复小组件（Tag 8 份、Pill 4 份、Empty 4 份），再补公司库编辑表单、Excel 批量导入、文件上传；然后 Max 灌真实数据 |
