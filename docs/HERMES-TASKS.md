...
> **顺序与质检协议已独立成文：`docs/DEV-ORDER-AND-QC.md`，以那份为准。**
> 本文只保留任务清单与通用铁律。
...
**更正（2026-09-17）**：此前这里列了「5.5 / 5.7 / 5.8 必须 Claude 做，因为需要设计判断」。
那个判断是错的 —— 六屏精修稿在阶段 2 就已定稿，剩下的是照稿实现，与 C 没有本质差别。
现在的分工是 **DS 全做、Claude 质检**，见 `docs/DEV-ORDER-AND-QC.md`。

真正不下放的只有两件：

| 项 | 为什么 |
|---|---|
| **仲裁** | DS 产出与既有决策冲突时谁说了算，由 Claude 判定 |
| **schema 迁移的设计** | 迁移脚本可以写，但改哪些列、为什么改，由 Claude 定。014 误删列的教训 |
