...
2026-09-17 新增的需求：Max 已完成项目（并购 / 收购 / 新品类开拓）的档案页，
项目背景 + 流程 + 结果，配图片和文档。现在 3 条，上限 20 条。

排在 E 之后，是因为它是**第一个从零开始、一次就写成 i18n 的页面** —— 前面的页面
都是先写死中文再回头接线，K 没有这个借口。与 F 无依赖，可以并行。

它也是本仓库第一个「文件即数据源」的页面：不建表、不写录入表单、不写索引脚本，
Max 直接在 `knowledge/` 下写 Markdown，页面扫目录渲染。20 条记录不值得一套 CRUD。

内容按公开性分两处放：图片是展会现场照片、本身就是开放数据，进 `public/knowledge/`
由 Next 静态发；正文与 `docs/`（合同、估值表）留在 `knowledge/`，文档走鉴权端点。
分界线只有一句：**能被陌生人看到也无所谓的，才放 `public/`** —— 那个目录不经
`proxy.ts` 中间件，页面受登录保护不等于页面里引用的静态文件受保护。
